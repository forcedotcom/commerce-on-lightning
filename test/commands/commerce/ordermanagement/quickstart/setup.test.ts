...
describe('Test order management setup command', () => {
    ...
    it('successfully sets up the order management', async () => {
        const usernameStub = org.getUsername.callsFake(() => 'test@1commerce.com');
        // @ts-ignore
        const connectionStub = org.getConnection.returns({
            sobject<T = object>(resource: string): SObject<T> {
                ...
                return {
                    ...
                    // @ts-ignore
                    select(
                        fields?: { [P in keyof T]: boolean } | Array<keyof T> | keyof T,
                        callback?: Callback<Array<Partial<T>>>
                    ): Query<Array<Partial<T>>> {
                        // @ts-ignore
    ...
    it('throws error when flows deployment fails', async () => {
        const usernameStub = org.getUsername.returns(username);
        const deployCommand = `sfdx force:mdapi:deploy -u "${username}" -d ${EXAMPLE_DIR}/som/ -w 1 --apiversion=${apiversion}`;
        ...
        [shellStub, usernameStub].forEach((stub) => stub.reset());
    });
    ...
    it('throws error when location api fails', async () => {
        ...
        [shellStub, usernameStub, connectionStub].forEach((stub) => stub.reset());
    });*/
});
